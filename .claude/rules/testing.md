...
- Assert only what the class under test is responsible for. If a value is produced by a
  collaborator (e.g. the format of an OTP generated by `OtpGenerator`), asserting its
  shape in the caller's test duplicates coverage that belongs to the collaborator's own test
  and couples the test to an implementation detail it does not own.
