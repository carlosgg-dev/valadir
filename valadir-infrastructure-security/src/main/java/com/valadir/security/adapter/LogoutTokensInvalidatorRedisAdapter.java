...
public class LogoutTokensInvalidatorRedisAdapter implements LogoutTokensInvalidator {
    ...
    // Atomic: blacklists the access token and removes the refresh token if already gone
    @Override
    public void invalidate(final String jti, final long remainingTtlSeconds, final String refreshToken) {

        redisTemplate.execute(
            logoutInvalidateTokensScript,
            List.of(RedisKeySpace.forBlacklist(jti), RedisKeySpace.forRefreshToken(refreshToken)),
